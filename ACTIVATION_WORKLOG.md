...
| 6 | **Silent referrer/UTM capture** on signup (no user-facing form) | ✅ DONE on PROD (2026-08-05) | `3bf93fc`, `43a1b65`, `3b460eb` | 8 nullable columns on `users` (`signup_referrer`, `signup_landing_path`, `signup_landing_querystring`, `utm_source/medium/campaign/content/term`), migration `f9708986d682` (down_revision `f5a6b7c8d9e0`, applied to staging DB). New `attribution.py`: HTTP middleware captures Referer + UTM + landing path into a short-lived HttpOnly cookie on a visitor's first real page GET (skips `/static`, `/api`, `/admin`, technical paths), **first-touch-wins** (never overwritten by later pages). Read back in both `create_user()` (password signup) and `_find_or_create_google_user`'s new-account branch (Google OAuth) — both signup paths covered; account-linking paths untouched. Missing/malformed cookie → all fields `None`, no crash. **Zero UX change: no new form fields, nothing rendered** — verified, not assumed. **QA'd real-browser, local server on `staging` branch + staging DB**: (1) landed with a real Referer (`reddit.com/r/3Dprinting`) + full UTM set on `/` → browsed to an unrelated page first → verified the capture cookie was **not** overwritten → signed up from `/signup` → re-queried the DB → every field (referrer, all 5 UTM params, landing_path=`/`, querystring) matched exactly. (2) Direct signup with no referrer/UTM straight to `/signup` → row created fine, all attribution fields `NULL`, `signup_landing_path`=`/signup`, no crash. (3) Signup form's visible fields confirmed byte-identical to before (`email`, `username`, `display_name`, `password`, `password_confirm` — nothing added). 18/18 real checks passed (1 test-script selector bug along the way — an overly broad CSS selector also matched the topbar's pre-existing search box — not a product issue, confirmed and corrected). Full 85-check suite re-run: 83/85, same 2 pre-existing external failures as before this work (Printables.com transiently 403ing our scraper UA — confirmed via direct curl, unrelated to any of these changes). All test accounts deleted from staging DB after, verified 0 residue. **DEPLOYED to prod 2026-08-05** — Cam approved; clean fast-forward merge staging→main (`21b817b`), Railway redeployed. Verified: all 8 attribution columns confirmed present on the prod `users` table via direct DB query (not inferred from app boot); health ok; `/admin` gates to `/login` (303, not 500). Capture is now live and recording every new signup; sources visible at `/admin#signup-sources`. |
| 7 | **Traffic-source investigation** (this wave) | ⏳ open | — | See findings below. Source still unknown; needs CF dashboard OR item #6 (now #8 gives Cam a UI to actually read it once #6 is on prod). |
| 8 | **Admin Signup Sources view** — see item #6's data in the console, not via SQL | ✅ DONE on PROD (2026-08-05) | `242dd46` | Added to the existing single-page admin console (`/admin`, same `_is_admin` gate + `admin-section`/`admin-table` pattern as Pro Members) — **appears at `/admin#signup-sources`**, right after the Growth section. Two pieces: (1) a rollup panel — referrer-domain and utm_source counts (grouped in Python, since raw referrer URLs can't be aggregated into domains portably in SQL; instant at current scale, same tradeoff as the Explore facets scan) plus a campaign/medium breakdown wherever `utm_source` is set, with a 7d/30d/90d/All-time window selector (`?days=`, defaults 30, invalid values fall back to 30). (2) the existing "Recent Signups" table gets two new columns — referrer domain + utm_source per row — so any individual signup is attributable at a glance. `_referrer_domain()` normalizes a Referer URL to just its host (www.-stripped), null/unparsable → `"(direct)"`. Read-only, reads only the #6 columns, no new PII. **QA'd real-browser, local server on `staging` branch + staging DB**: seeded 6 users with distinct referrer/utm combos (incl. one backdated 100 days to prove the window filter genuinely filters, one pure direct/null) + a fresh admin account + a non-admin account — **35/35 checks passed**: rollup counts matched exactly (reddit.com ×2, correct utm_source/campaign groupings), window selector correctly excluded/included the backdated seed at 30d vs All-time, all 6 per-user table rows matched their seeded source exactly, non-admin redirected away from `/admin`, logged-out redirected to `/login`. All 8 test accounts deleted after, verified 0 residue. Full 85-check suite re-run: 83/85, same 2 pre-existing external failures (Printables.com), unrelated. **DEPLOYED to prod 2026-08-05** — Cam approved; clean fast-forward merge staging→main (`21b817b`), Railway redeployed. Verified: all 8 attribution columns confirmed present on the prod `users` table via direct DB query (not inferred from app boot); health ok; `/admin` gates to `/login` (303, not 500). Capture is now live and recording every new signup; sources visible at `/admin#signup-sources`. |
