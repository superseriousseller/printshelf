...
| 5 | **Email reorder** — verify-at-signup; welcome fires on verify success (not both at signup) | 🔷 BUILT + QA'D, STAGED (2026-08-05) | `052c132` | `send_welcome` moved out of `signup_submit` into `verify_email`'s success branch (gated by the existing `used_at` replay check, so it can't double-fire). Google OAuth signups unaffected — they verify inline and already send welcome separately in the callback. **QA'd on `staging.printshelf.app` against real Resend send logs (not just code reading)**: real-browser signup → Resend API confirmed **exactly 1 email** (verification only, no welcome); extracted the real verify link from the actual sent email body via the Resend API; browser-clicked it → verification success page; re-checked Resend → welcome email now present, **exactly once**, verification email still exactly once (no duplicates) — signup-time sends go from 2 to 1, welcome now fires only once verification actually completes. 10/10 checks passed. Test account fully deleted from staging DB after. **Committed on `staging` only — not merged to main, not on prod.** |
