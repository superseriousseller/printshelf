...
**Immediate next step:** Set CF proxy env vars on Railway prod, then next feature — Cam's call. Remaining candidates (engagement loop + cold-start + PWA + facets + data-export now DONE): OAuth/Google social login (signup-funnel friction — needs Cam to provision Google OAuth creds first); collections/named lists beyond the 11 fixed categories; **brand-name normalization** (facet options + filament data show casing dupes like `Bambu`/`BAMBULAB`/`Bambu Labs`, `Hatchbox`/`HATCHBOX` — surfaced by the facets feature); explore facets scale fix (`print_filaments` assoc table to replace the in-memory JSON-array scan).
