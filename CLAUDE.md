...
**Immediate next step:** Set CF proxy env vars on Railway prod, then next feature — Cam's call. Remaining gap-analysis candidates (session 28 inventory, obs 9723; engagement loop + cold-start + PWA now DONE): search/explore facets (material/brand/printer over data already stored); user data export (CSV/JSON before account-delete, also a GDPR/portability gap); OAuth/Google social login (signup-funnel friction); collections/named lists beyond the 11 fixed categories.
