...
**Immediate next step:** Set CF proxy env vars on Railway prod, then next feature — Cam's call. Top gap-analysis candidates (session 28 inventory, obs 9723): feed cold-start fallback to global when following nobody (XS); PWA/installable manifest (S, retention for a logging app); time-decayed "Trending" sort (now that likes have timestamps); search/explore facets (material/brand/printer); user data export (CSV/JSON before account-delete).
